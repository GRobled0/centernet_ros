...
def ctdetplus_post_process(dets, c, s, h, w, num_classes): #NUEVA
  # dets: batch x max_dets x dim
  # return 1-based class det dict
  ret = []
  for i in range(dets.shape[0]):
    ...
    classes = dets[i, :, -2] #añadi en ultimo lugar la profundidad asi q las clases son la penultipa variable

    for j in range(num_classes):
      inds = (classes == j)
      top_preds[j + 1] = np.concatenate([
        dets[i, inds, :4].astype(np.float32), #bbox
        dets[i, inds, 4:5].astype(np.float32), #score
        get_pred_depth(dets[i, inds, 6:7]).astype(np.float32)], axis=1).tolist() #profundidad
    ret.append(top_preds)
  return ret
