...
def ctdetplus_decode(depth, heat, wh, reg=None, cat_spec_wh=False, K=100): #NUEVO
    batch, cat, height, width = heat.size()
    ...
    if reg is not None:
      ...
      ys = ys.view(batch, K, 1) + 0.5
    wh = _transpose_and_gather_feat(wh, inds)
    depth = _transpose_and_gather_feat(depth, inds) #añadido
    depth = depth.view(batch, K, 1)
    if cat_spec_wh:
      wh = wh.view(batch, K, cat, 2)
      clses_ind = clses.view(batch, K, 1, 1).expand(batch, K, 1, 2).long()
    ...
    bboxes = torch.cat([xs - wh[..., 0:1] / 2, 
                        ...
                        xs + wh[..., 0:1] / 2, 
                        ys + wh[..., 1:2] / 2], dim=2)
    detections = torch.cat([bboxes, scores, clses, depth], dim=2) #añadida la profundidad
